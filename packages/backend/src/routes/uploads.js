...
// Strip the extension and sanitize a filename so it's safe for Cloudinary's
// public_id / filename_override (no spaces, accents, or special characters).
function sanitizeFilename(originalname = '') {
  const base = path.parse(originalname).name; // filename without extension
  return base
    .normalize('NFKD')
    ...
    .slice(0, 80) || 'file';          // cap length, fallback if empty
}
  ...
  async (req, res) => {
    try {
      ...
      const uploadOptions = {
        ...
        // use_filename alone doesn't work for buffer/stream uploads — Cloudinary
        // has no real filename to read, so it falls back to an autogenerated
        // name (e.g. "file_xxxxx"). filename_override explicitly supplies the
        // name we want, derived from the original upload.
        use_filename:      true,
