...
export default function RegisterPage() {
  const [form, setForm] = useState({
    ...
    category: 'Ordinary',   // new: membership category
    initialBand: '',        // optional: band they want to join
  });
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4" style={{ background: '#F5F7FF' }}>
      <div className="text-center p-8 rounded-2xl bg-white max-w-md" style={{ boxShadow: '0 8px 30px rgba(0,0,0,0.08)' }}>
        ...
        <h2 className="text-2xl font-bold mb-4" style={{ color: '#0F2A4A' }}>Registration Submitted</h2>
        <p style={{ color: '#475569', lineHeight: 1.7, marginBottom: 24 }}>
          Your account has been created but requires admin approval. You will receive an email once your account is approved.
        </p>
  ...
  const inputStyle = { width: '100%', padding: '12px 14px', borderRadius: 10, fontSize: 14, border: '1px solid #CBD5E1', background: '#FFFFFF', color: '#0F172A', outline: 'none', fontFamily: "'Noto Sans', sans-serif" };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 py-10" style={{ background: '#F5F7FF' }}>
      <div className="w-full max-w-md rounded-2xl p-8" style={{ background: 'white', boxShadow: '0 8px 30px rgba(0,0,0,0.08)' }}>
        ...
        <p className="text-center mb-6" style={{ color: '#475569', fontSize: 13 }}>Create your student account to get started.</p>

        {error && <div className="rounded-lg p-3 mb-4 text-sm" style={{ background: '#FEF2F2', color: '#DC2626', border: '1px solid #FEE2E2' }}>{error}</div>}

        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <div><label style={labelStyle}>Full Name *</label><input required placeholder="John Mwanza" value={form.name} onChange={set('name')} style={inputStyle} /></div>
          ...
          <div><label style={labelStyle}>Password * (min. 6 characters)</label><input required type="password" placeholder="••••••••" value={form.password} onChange={set('password')} style={inputStyle} /></div>
          ...
          {form.category === 'Ordinary' && (
            <>
              <div><label style={labelStyle}>Student ID</label><input placeholder="MU2024/001" value={form.studentId} onChange={set('studentId')} style={inputStyle} /></div>
              <div><label style={labelStyle}>Department / School</label><input placeholder="School of Engineering" value={form.department} onChange={set('department')} style={inputStyle} /></div>
              <div><label style={labelStyle}>Year of Study</label><input placeholder="Year 2" value={form.year} onChange={set('year')} style={inputStyle} /></div>
            </>
          )}

          <div><label style={labelStyle}>Phone (optional)</label><input type="tel" placeholder="+260 977 123 456" value={form.phone} onChange={set('phone')} style={inputStyle} /></div>
          ...
          <button type="submit" disabled={loading} className="w-full py-3 rounded-xl text-sm font-bold text-white mt-2" style={{ background: loading ? '#94A3B8' : '#2E6DE7' }}>{loading ? 'Creating your account...' : 'Sign Up'}</button>
        </form>

        <p className="text-center text-xs mt-4" style={{ color: '#64748B' }}>Already have an account?{' '}<a href="/login" className="font-semibold" style={{ color: '#2E6DE7' }}>Log in</a></p>
        <p className="text-center text-xs mt-2"><a href="/dashboard" style={{ color: '#2E6DE7' }}>Continue as guest</a></p>
      </div>
    </div>
  );
}