  ...
  return (
    <div className="rounded-2xl overflow-hidden flex flex-col transition-all duration-200 hover:-translate-y-0.5"
      ...
      <div className="px-6 pb-5">
        <button onClick={() => onJoin(group)} className="w-full py-2.5 rounded-xl text-sm font-semibold transition-all"
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(15,42,74,0.6)', backdropFilter: 'blur(4px)' }}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="px-6 py-5 flex flex-col gap-4">
          ...
          <div>
            <label style={{ display: 'block', fontSize: 10, fontWeight: 700, letterSpacing: '0.1em', color: '#0F2A4A', marginBottom: 6 }}>WHY DO YOU WANT TO JOIN?</label>
            <textarea value={form.motivation} onChange={f('motivation')} rows={3} placeholder="Share your motivation…" style={{ ...inputStyle, resize: 'none' }} />
          </div>
...
export default function GroupsPage() {
  ...
  const filtered = list => list.filter(g => g.name.toLowerCase().includes(search.toLowerCase()) || (g.leader || '').toLowerCase().includes(search.toLowerCase()));
  ...
  return (
    <>
      ...
      <div style={{ minHeight: '100vh', background: '#FFFFFF', color: '#1E293B' }}>
        ...
        <PageHeader eyebrow="MU SDA PCM" title="Campus Groups & Departments" subtitle="Find your place to serve. Every group is a family — discover where your gifts can shine." />
        <div className="max-w-7xl mx-auto px-5 sm:px-8 py-16">
          <div className="rounded-2xl p-6 mb-12 grid grid-cols-3 gap-6 text-center" style={{ background: '#F5F7FF', border: '1px solid #E2E8F7' }}>
            {[{ value: active.length, label: 'Active Groups' }, { value: `${totalMembers}+`, label: 'Total Members' }, { value: groups.length, label: 'Departments' }].map(({ value, label }) => (
              <div key={label}>
